...
It was termed Good-Turing Discounting (also known as Good-Turing smoothing), it became a technique to re-estimate probability mass to assign N-Grams with zero or low counts by discounting from those occurring more often.
...
Codes to remove the processed docs from corpus, provide user progress, remove the "words" <eos> and <num> from the table and create a loop to process the 10,000 docs:
...
Codes to build the three-gram model. The deal here is to break large corpus into chunks to deal with limitations imposed by RAM in R:
...
Codes to repeat the above loop for the remaining documents (partial step):
...
Final phases of the aggregation process to build a three-gram dataframe.
